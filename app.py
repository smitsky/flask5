...
# --- CRITICAL FIX: DB Initialization Order (Must be here) ---
db = SQLAlchemy(app)
...
class User(UserMixin, db.Model):
    # CRITICAL FIX: Explicitly use 'users' to match your database and avoid the PostgreSQL reserved keyword 'user'
    __tablename__ = 'users' 
    ...
    def needs_rehash(self, target_iterations=None):
        ...
        try:
            method_part = self.password_hash.split('$', 1)[0]
...
# CLI command to create the tables (using 'users' table name)
